var canvas = document.getElementById('canvas'),
  ...
  activeColor = 'rgba(128,255,255,0.5)',
  ...
  LEFT_MARGIN = 10,
  FRAME_LINE_WIDTH = 10,
  ROD_STROKE_STYLE = 'rgba(128,255,255,0.5)', //الحبل
  ROD_LINE_WIDTH = 6,
  DOT_STROKE_STYLE = 'rgba(0, 0, 0, 1)', //النقط البيضة
...
Abacus.prototype = {
  drawFrame: function () {
    ...
    context.lineTo(
      LEFT_MARGIN + this.width - FRAME_LINE_WIDTH / 2,
      top_frame + HEAVEN
    );
    context.stroke();
    ...
    for (
      var i = 0, x = LEFT_MARGIN + DISTANCE_RODS;
      i < this.numberOfRods;
      ++i, x += DISTANCE_RODS
    ) {
...
Rod.prototype = {
  ...
  writeValue: function () {
    ...
    context.fillStyle = 'rgba(255,255,255,255)'; // لون الارقام
...
Bead.prototype = {
  getPoints: function () {
    ...
    points.push(
      new Point(center.x + BEAD_WIDTH / 6, center.y - BEAD_HEIGHT / 2)
    );
    // ____\
  ...
  evalPosition: function () {
    // returns the central point of the bead;
    var x = LEFT_MARGIN + this.rod.position * DISTANCE_RODS,
      y = undefined;
...
var lastMessageDate = localStorage.getItem('lastMessageDate');
